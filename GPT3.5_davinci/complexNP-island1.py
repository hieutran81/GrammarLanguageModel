...
INPUT = pandas.read_excel("input_GPT3_complexNP-island1.xlsx")
...
for row in range(84):
  ...
  print(response.choices[0].text)
  if row <=20:
    df = pandas.DataFrame({'item': [row+1], 'Correct': [response.choices[0].text==(" \""+C1+"\"")], 'gap': ['no'], 'island': ['obj_noisland'], 'C1':[C1], 'C2':[C2], 'Response':[response.choices[0].text.lstrip("\n")]})
    RESULTS = pandas.concat([RESULTS,df])
  else:
    if row <=41:
      df = pandas.DataFrame({'item': [row-20], 'Correct': [response.choices[0].text==(" \""+C2+"\"")], 'gap': ['yes'], 'island': ['obj_noisland'], 'C1':[C1], 'C2':[C2], 'Response':[response.choices[0].text.lstrip("\n")]})
      RESULTS = pandas.concat([RESULTS,df])
    else:
      if row <=62:
        df = pandas.DataFrame({'item': [row-41], 'Correct': [response.choices[0].text==(" \""+C1+"\"")], 'gap': ['no'], 'island': ['rc_obj_island'], 'C1':[C1], 'C2':[C2], 'Response':[response.choices[0].text.lstrip("\n")]})
        RESULTS = pandas.concat([RESULTS,df])
      else:
        if row<=83:
          df = pandas.DataFrame({'item': [row-62], 'Correct': ['NA'], 'gap': ['yes'], 'island': ['rc_obj_island'], 'C1':[C1], 'C2':[C2], 'Response':[response.choices[0].text.lstrip("\n")]})
          RESULTS = pandas.concat([RESULTS,df])
print(RESULTS)
RESULTS.to_csv("complexNP_island_continuation1.csv")
  ...
  if row <=20:
    ...
    if row <=41:
      ...
      if row <=62:
        ...
        if row<=83:
          df = pandas.DataFrame({'item': [row-62], 'Correct': [response.choices[0].text=="Neither sequence is a complete sentence."], 'gap': ['yes'], 'island': ['rc_obj_island'], 'C1':[C1], 'C2':[C2], 'Response':[response.choices[0].text.lstrip("\n")]})
          RESULTS = pandas.concat([RESULTS,df])
print(RESULTS)
RESULTS.to_csv("complexNP_island_completeness1.csv")
