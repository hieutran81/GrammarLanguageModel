...
import openai
import pandas
openai.api_key = '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'
openai.Model.list()


INPUT = pandas.read_excel("input_GPT3_adjunct-island.xlsx")
...
for row in range(80):
  ...
  print(response.choices[0].text)
  if row <=19:
    df = pandas.DataFrame({'item': [row+1], 'Correct': [response.choices[0].text==(" \""+C1+"\"")], 'gap': ['no'], 'island': ['modifier_noisland'], 'C1':[C1], 'C2':[C2], 'Response':[response.choices[0].text.lstrip("\n")]})
    RESULTS = pandas.concat([RESULTS,df])
  else:
    if row <=39:
      df = pandas.DataFrame({'item': [row-19], 'Correct': [response.choices[0].text==(" \""+C2+"\"")], 'gap': ['yes'], 'island': ['modifier_noisland'], 'C1':[C1], 'C2':[C2], 'Response':[response.choices[0].text.lstrip("\n")]})
      RESULTS = pandas.concat([RESULTS,df])
    else:
      if row <=59:
        df = pandas.DataFrame({'item': [row-39], 'Correct': [response.choices[0].text==(" \""+C1+"\"")], 'gap': ['no'], 'island': ['adjunct'], 'C1':[C1], 'C2':[C2], 'Response':[response.choices[0].text.lstrip("\n")]})
        RESULTS = pandas.concat([RESULTS,df])
      else:
        df = pandas.DataFrame({'item': [row-59], 'Correct': ['NA'], 'gap': ['yes'], 'island': ['adjunct'], 'C1':[C1], 'C2':[C2], 'Response':[response.choices[0].text.lstrip("\n")]})
        RESULTS = pandas.concat([RESULTS,df])
print(RESULTS)
RESULTS.to_csv("adjunct_island_continuation.csv")
  ...
  if row <=19:
    ...
    if row <=39:
      ...
      if row <=59:
        ...
        df = pandas.DataFrame({'item': [row-59], 'Correct': [response.choices[0].text=="Neither is grammatical."], 'gap': ['yes'], 'island': ['adjunct'], 'C1':[C1], 'C2':[C2], 'Response':[response.choices[0].text.lstrip("\n")]})
        RESULTS = pandas.concat([RESULTS,df])
print(RESULTS)
RESULTS.to_csv("adjunct_island_grammaticality.csv")
"""
  ...
  if row <=19:
    ...
    if row <=39:
      ...
      if row <=59:
        ...
        df = pandas.DataFrame({'item': [row-59], 'Correct': [response.choices[0].text=="Neither sequence is a complete sentence."], 'gap': ['yes'], 'island': ['adjunct'], 'C1':[C1], 'C2':[C2], 'Response':[response.choices[0].text.lstrip("\n")]})
        RESULTS = pandas.concat([RESULTS,df])
print(RESULTS)
RESULTS.to_csv("adjunct_island_completeness.csv")
